...
def create_vector_store(documents: List[Document], api_key: str):
    ...
    class EnhancedVectorStore:
        ...
        def similarity_search(self, query, k=5):  # Increased k for better retrieval
            ...
            # Return documents with scores - more lenient threshold
            results = []
            for i, (score, idx) in enumerate(zip(D[0], I[0])):
                if idx < len(self.documents) and score > 0.01:  # Lower threshold for better recall
                    results.append(self.documents[idx])
            
            # If no results with threshold, return top 2 anyway
...
1. Answer based on the information provided in the documents
2. If you can find relevant information, provide a helpful answer even if it's partial
...
6. If the documents contain general information that might be relevant, share it
...
Please provide a helpful answer based on the information in the documents above:"""
...
def answer_question(qa_chain, question: str) -> tuple:
    ...
    try:
        ...
        # Enhanced check for relevant source documents - more lenient
        is_found_in_docs = (
            ...
            not any(phrase in answer.lower() for phrase in [
                ...
                "i cannot find", "there is no", "not available"
            ])
        )
