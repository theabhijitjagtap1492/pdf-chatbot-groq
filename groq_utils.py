...
def create_vector_store(documents: List[Document], api_key: str):
    ...
    class EnhancedVectorStore:
        ...
        def similarity_search(self, query, k=10):  # Increased k for better retrieval
            ...
            # Return documents with scores - adaptive threshold
            results = []
            for i, (score, idx) in enumerate(zip(D[0], I[0])):
                if idx < len(self.documents) and score > 0.02:  # Lower threshold but more results
                    results.append(self.documents[idx])
            
            # Return top 5 most relevant for better context
            return results[:5]
...
def get_qa_chain(vector_store, api_key: str):
    ...
    class EnhancedQAChat:
        ...
        def __call__(self, inputs):
            ...
            prompt = f"""You are a precise AI assistant that answers questions based on the provided documents.

CRITICAL INSTRUCTIONS:
1. ONLY answer based on the EXACT information provided in the documents
2. If the answer is not clearly stated in the documents, say "I cannot find specific information about this in the provided documents"
3. Do NOT make assumptions or add information not present in the documents
4. Quote exact text from the documents when possible
5. If you're unsure, say so clearly
6. Be concise and accurate
7. If multiple documents contain relevant information, combine them clearly
8. Always cite which document the information comes from
...
Provide a precise answer based ONLY on the documents above:"""
...
def answer_question(qa_chain, question: str) -> tuple:
    ...
    try:
        ...
        # Enhanced check for relevant source documents - more precise
        is_found_in_docs = (
            ...
            not any(phrase in answer.lower() for phrase in [
                ...
                "there doesn't appear to be", "no specific information"
            ]) and
            len(answer.strip()) > 30  # Ensure answer has meaningful content
        )
