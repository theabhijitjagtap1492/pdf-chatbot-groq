...
def create_vector_store(documents: List[Document], api_key: str):
    ...
    class EnhancedVectorStore:
        ...
        def similarity_search(self, query, k=20):  # Even more results for better understanding
            ...
            # Return documents with scores - extremely low threshold for maximum recall
            results = []
            for i, (score, idx) in enumerate(zip(D[0], I[0])):
                if idx < len(self.documents) and score > 0.001:  # Extremely low threshold
                    results.append(self.documents[idx])
            
            # If still no results, return top 5 anyway
            ...
            # Return top 10 most relevant for better context
            return results[:10]
        ...
        def _expand_query(self, query):
            ...
            # Always add some general terms for better search
            general_terms = ['information', 'content', 'data', 'text', 'document']
...
def get_qa_chain(vector_store, api_key: str):
    ...
    class EnhancedQAChat:
        ...
        def __call__(self, inputs):
            ...
            prompt = f"""You are a helpful AI assistant that understands and answers questions based on the provided documents.

IMPORTANT INSTRUCTIONS:
1. First, understand what the user is asking for - even basic questions like "what is the name" or "what types"
2. Look for ANY relevant information in the documents that could answer their question
3. If you find relevant information, provide a clear and helpful answer
4. If the information is not in the documents, say "I cannot find specific information about this in the provided documents"
5. Be helpful and informative - if you can answer part of the question, do so
6. For basic questions, look for titles, names, types, categories, lists, or any identifying information
7. Quote relevant text from the documents when helpful
8. If multiple documents contain relevant information, combine them clearly
9. Always be clear about what information you found and what you didn't find
